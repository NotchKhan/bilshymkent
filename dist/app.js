...
const slideImages=["/assets/banner-city.jpg","/assets/banner-kz.jpg","/assets/banner-values.jpg"];
slideImages.slice(1).forEach(src=>{const image=new Image();image.src=src;});
...
function showSlide(next=slide){const list=slides[currentLanguage];slide=(next+list.length)%list.length;const data=list[slide];document.querySelector(".hero-image").src=slideImages[slide];document.getElementById("hero-title").innerHTML=data.title;document.getElementById("hero-description").innerHTML=data.description;const link=document.getElementById("hero-link");link.href=data.link;link.querySelector("span").textContent=data.label;document.getElementById("slide-count").textContent=(slide+1)+" / "+list.length;}
